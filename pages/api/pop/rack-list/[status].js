...
async function rackList(req, res) {
  ...
  if (!session) {
    res.status(401).send({ error: 'You must be sign in to view the protected content on this page.' })
  }
  ...
  const API_URL = process.env.POP_API_URL + '?status=' + (status ? status : '-1')
  ...
  if (jsonData.errors) {
    res.status(500).json(json.errors)
  }
